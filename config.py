...
MB_VOLUME_RATIO_MIN      = 1.0    # today_vol / yesterday_vol gate (secondary check, kept for continuity)
MB_BREAKOUT_VOL_MIN_RATIO = 1.5  # FIX B: today_vol must be ≥ 1.5× 50-day avg volume (primary check)
                                  # PB: "Volume must confirm the burst — not just beat yesterday"
...
MB_CONSOL_MIN_WIDTH_PCT  =  7.0   # FIX: added (backtest: 7-10% = 55.8% WR; <7% = <40% WR)
...
TI65_BULL_THRESHOLD   = 1.05  # FIX: raised 1.03→1.05 (backtest: >1.05=49% WR vs 1.03-1.05=38.9%)
...
RS_MIN_FOR_MB    = 80     # FIX: raised 70→80 (backtest: RS 80-99=61% WR vs RS 60-80=48.5%)
...
MB_SCORE_THRESHOLDS = {
    "elite":  85,
    "strong": 70,
    "watch":  65,   # FIX: raised 55→65 (force higher quality in NEUTRAL-dominated market)
}
