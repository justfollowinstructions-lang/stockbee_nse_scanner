...
def _download_batch_v2(
    ...
    # FIX: end date was date.today() + 1 day, which asks Yahoo for a
    # trading day that hasn't happened yet (e.g. tomorrow). Yahoo's `end`
    # param is exclusive of that calendar date, so to get TODAY's bar
    # (if market has closed) you only need end = tomorrow when running
    # AFTER close. Running this manually/intra-day, asking for "tomorrow"
    # causes every symbol to come back empty with "possibly delisted" and
    # burns 3 retries per symbol (~60s each) for nothing.
    # Correct: just ask for end = today. yfinance returns whatever bars
    # exist up to and including today — exactly "all data available so far".
    end = date.today().isoformat()

    # FIX: a symbol whose last saved bar is already today has
    # _incremental_start() == today+1, which is now AFTER `end`.
    # Asking yfinance for start > end always returns empty and would
    # burn 3 retries for nothing. Skip these up front — they're already
...
def _download_single_with_retry(symbol: str) -> bool:
    ...
    # FIX: if last saved bar is already today, _incremental_start() returns
    # today+1 which is now after `end`. That's not a failure — it's already
    # current. Without this check we'd burn 3 retries (~30-45s) per symbol
    # logging a false "Failed to download" error for data we already have.
    ...
    for attempt in range(1, 4):
        try:
            ...
            # FIX: same bug as batch path — was asking for tomorrow.
            # "Today" gets whatever is actually available right now.
            df = ticker.history(
                start=sym_start,
                end=end_str,
                auto_adjust=True,
