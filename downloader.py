...
def _download_batch_v2(
    ...
    # yfinance's `end` parameter is EXCLUSIVE of the calendar date given.
    # To ever receive today's bar (once Yahoo has published it, i.e. after
    # ~4pm IST close), `end` MUST be tomorrow. Setting end=today excludes
    # today's bar entirely and was the cause of every symbol — including
    # liquid largecaps like TCS/INFY/HDFCBANK — coming back empty when run
    # mid-session, because the window collapsed to zero width.
    #
    # The earlier "possibly delisted" issue (asking for 2026-07-01 future
    # data) was NOT actually caused by end=tomorrow being wrong in general —
    # it was the date.today() computation drifting across a day boundary on
    # the runner. end=tomorrow is the correct, standard yfinance idiom.
    end = (date.today() + timedelta(days=1)).isoformat()

    # A symbol whose last saved bar is already TODAY has
    # _incremental_start() == tomorrow, which now equals `end`.
    # start == end (or start > end) means nothing new to fetch — skip it,
    # this is "already current," not a failure.
    already_current = [sym for sym in batch if starts[sym] >= end]
    batch = [sym for sym in batch if sym not in already_current]
    if already_current:
        log.debug("Already up to date, skipping: %s", already_current[:5])

    if not batch:
        return len(already_current), [], []

    # Find the earliest start (to make one call covering the full range)
    # Each symbol's data will be sliced after extraction
    min_start = min(starts[sym] for sym in batch)
    ...
    try:
        raw = yf.download(
            ...
            auto_adjust=True,
            progress=False,
            threads=False,   # threads=False is more stable for large batches
        )
    ...
    if raw is None or raw.empty:
        # The whole batch came back empty. Two possible causes:
        #   1. Run mid-session (e.g. 2pm IST) — today's bar genuinely
        #      doesn't exist in Yahoo's feed yet. This is NORMAL, not a
        #      failure, and should not be retried 3x for every symbol.
        #   2. Real network/Yahoo outage — a genuine failure.
        # Distinguish by retrying once with end=today (excludes today's
        # date entirely). If THAT succeeds, case 1 is confirmed — every
        # symbol simply has no new bar yet, which is a clean no-op.
        fallback_end = date.today().isoformat()
        if fallback_end != min_start:   # only retry if there's a non-empty window
            try:
                raw_fallback = yf.download(
                    batch,
                    start=min_start,
                    end=fallback_end,
                    auto_adjust=True,
            ...
            if raw_fallback is not None and not raw_fallback.empty:
                log.info(
                    "Batch empty for end=%s (today's bar not yet published — "
                    "mid-session run). Falling back to data through yesterday "
                    "for %d symbols.", end, len(batch)
                )
                raw = raw_fallback
            else:
    ...
    return saved + len(already_current), timed_out, rate_limited
...
def _download_single_with_retry(symbol: str) -> bool:
    ...
    # end is exclusive — must be tomorrow to ever capture today's bar
    # once it's published (after ~4pm IST close).
    end_str = (date.today() + timedelta(days=1)).isoformat()

    # If last saved bar is already today, incremental_start() == tomorrow
    # == end_str. Nothing new to fetch — already current, not a failure.
    if sym_start >= end_str:
        log.debug("%s: already up to date (last bar >= today)", symbol)
        return True
    ...
    for attempt in range(1, 4):
        try:
            ...
            if df.empty:
                # On the FIRST attempt, an empty result with end=tomorrow
                # most likely means today's bar genuinely isn't published
                # yet (mid-session run). Try once more with end=today
                # (excludes today) — if that has data, it's a clean
                # "nothing new yet" case, not a failure worth retrying.
                if attempt == 1:
                    fallback_end = date.today().isoformat()
                    if fallback_end != sym_start:
                        df_fb = ticker.history(
                            start=sym_start,
                            end=fallback_end,
                            auto_adjust=True,
                        )
                        if not df_fb.empty:
                            ...
                            log.debug("%s: no new bar yet (mid-session) — skipping", symbol)
                            return True
                    else:
                        log.debug("%s: no new bar yet (mid-session) — skipping", symbol)
                        return True
