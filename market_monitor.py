...
class MarketMonitorSnapshot:
    ...
    trading_allowed:         bool    # False in BEAR
    ...
    def _build_summary(self) -> str:
        ...
        return (
            ...
            f"Regime Score: {self.regime_score:.0f}/100 | "
            f"{'✅ BUY AGGRESSIVELY' if self.buy_breakouts_aggressively else '⚠️ BE SELECTIVE' if self.trading_allowed else '🚫 AVOID LONGS'}"
        )
...
def compute_market_monitor(
    ...
    wk_up_pct = wk_up20  / universe_size
    ...
    trading_allowed  = (regime in ("BULL", "NEUTRAL"))
...
def _compute_regime_score(
    ...
    # 52W highs / lows (10 pts): more highs + fewer lows = better
...
def market_allows_trading(
    ...
    """
    PB's trading permission gate.

    MB setups:  require BULL or NEUTRAL
    EP setups:  can trade even in CAUTION (catalyst overrides market)
...
def generate_market_monitor_report(snapshot: MarketMonitorSnapshot) -> str:
    ...
    if snapshot.buy_breakouts_aggressively:
        ...
        if snapshot.market_regime == "CAUTION":
            lines.append("   EP setups allowed | MB setups: avoid")
    else:
...
def weekly_breadth_scan(
    ...
    result["weekly_winners"] = (
        wk[wk["wk_chg"] >= 20].nlargest(top_n, "wk_chg")
        .to_dict("records")
    )
    result["weekly_losers"] = (
        wk[wk["wk_chg"] <= -20].nsmallest(top_n, "wk_chg")
        .to_dict("records")
    )
    ...
    result["monthly_leaders"] = (
        mo[mo["mo_chg"] >= 50].nlargest(50, "mo_chg")
        .to_dict("records")
    )
