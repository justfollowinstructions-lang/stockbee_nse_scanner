...
def run_backtest(
    ...
    for day_idx, sim_date in enumerate(trading_days):
        ...
        for sym, trade in open_positions.items():
            ...
            # Intraday stop hit
            stop_hit = today_low <= trade.stop_loss

            # Day 3 partial exit for MB — only records if +8% target is hit
            # FIX: was unconditionally setting partial_exit_price = today_close
            # on every Day 3, making day3_target_hit_pct = 98.8% (meaningless).
            # PB's rule: exit 50% on Day 3 IF price is at/above +8% target.
            # If not there yet, hold and let the Day 5 force exit handle it.
            if ("MB" in trade.signal_type and
                    hold_days >= MB_PARTIAL_EXIT_DAY - 1 and
                    trade.partial_exit_price is None and
                    today_close >= trade.entry_price * 1.08):   # FIX: must be at +8%
            ...
            if stop_hit:
                ...
                today_close = trade.stop_loss   # simulate stop fill at stop price
            ...
            # EP trailing stop: after 20%+ gain, trail at 10% below recent high
                ...
                if gain_pct >= 20.0:
                    # Recalculate recent high (last 10 bars)
                    ...
                    if today_low <= trail_stop:
                        ...
                        today_close = trail_stop

            if force_exit:
                trade.exit_date   = sim_date
                trade.exit_price  = round(today_close, 2)
...
def _compute_metrics(
    ...
    # Stockbee-specific MB metrics
    day3_hit_pct   = 0.0
    burst_8pct_pct = 0.0
    mb_trades = df[df["signal_type"].str.contains("MB", na=False)].copy()
    if len(mb_trades) > 0:
        # Day 3 target hit = partial_exit_price was set (meaning +8% was reached
        # by Day 3 or sooner). FIX: was measuring hold_days <= 3, which counted
        # any short trade (including stops) as a "Day 3 hit" → 98.8% false rate.
        if "partial_exit_price" in mb_trades.columns:
            day3_hit_pct = float(mb_trades["partial_exit_price"].notna().mean() * 100)
        # Burst 8%+: stock reached +8% OR more at any point during hold
        # Uses max_gain_pct if available, else falls back to exit pnl
        if "max_gain_pct" in mb_trades.columns:
            burst_8pct_pct = float((mb_trades["max_gain_pct"] >= 8.0).mean() * 100)
        else:
