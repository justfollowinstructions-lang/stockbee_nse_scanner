...
def detect_consolidation(
    ...
    for length in range(MB_CONSOL_MIN_BARS, MB_CONSOL_MAX_BARS + 1):
        ...
        if (pct_chg < MB_CONSOL_NEG_DAY_PCT).any():
            break   # longer windows will have same problem
...
def detect_mb_signal(
    ...
    breakout_pct = (today_close - prev_close) / prev_close * 100
    if breakout_pct < MB_BREAKOUT_PCT:
        log.debug("%s: skip MB — breakout %.2f%% < threshold %.1f%%",
                  symbol, breakout_pct, MB_BREAKOUT_PCT)
        return None

    # ── 5. Volume: today > yesterday ─────────────────────────────────────────
    today_vol = float(volume.iloc[sig_idx])
    prev_vol  = float(volume.iloc[sig_idx - 1])
    vol_ratio = today_vol / prev_vol if prev_vol > 0 else 0.0
    if vol_ratio < MB_VOLUME_RATIO_MIN:
        log.debug("%s: skip MB — volume ratio %.2f < %.1f", symbol, vol_ratio, MB_VOLUME_RATIO_MIN)
        return None
