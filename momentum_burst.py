...
def detect_consolidation(
    ...
    for length in range(MB_CONSOL_MIN_BARS, MB_CONSOL_MAX_BARS + 1):
        ...
        # FIX (backtest data): 7-10% width = 55.8% WR, <7% = <40% WR.
        # Too-tight consolidations (<7%) often aren't real bases — more like
        # noise/illiquidity, not genuine institutional accumulation.
        ...
        if width_pct > MB_CONSOL_MAX_WIDTH_PCT:
            continue
        if width_pct < MB_CONSOL_MIN_WIDTH_PCT:
            continue   # FIX: reject overly tight bases — backtest-validated
